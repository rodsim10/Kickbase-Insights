...
def market(user_token: str, selected_league: object, own_user_id: str) -> None:
    ...
    for player in players_on_market:
        ...
        ### Kickbase only sends an expiry ("exs") for its own listings, never for player ones
        ...
        player_info = {
            ...
            "isFreePlayer": not player.username,
    ...
    free_players = sum(1 for player in players_on_the_market if player["isFreePlayer"])
    ...
    logging.info(f"Got all {len(players_on_the_market)} players listed on transfer market "
                 f"({free_players} listed by Kickbase, {own_bids} with a bid of yours).")
